...
function Expenses(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    api.categories.list('expense')
      ...
      .catch((err: unknown) => {
        logToMain('error', 'load expense categories failed', { message: (err as ApiError).message })
      })
  ...
  const load = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const message = (err as ApiError).message ?? 'Expense data could not be loaded.'
      setError(message)
      logToMain('error', 'load expenses page failed', { message })
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="page">
      ...
      <div className="card page-toolbar">
        ...
        {categories.length > 0 && (
          <label className="field__label page-cat-filter" htmlFor="expense-cat-filter">
